function snow() {
    let flakeList = []

    function update() {
        flakeList.forEach((info, i) => {

            let {flake, horizontalSpeed, verticalSpeed, disabled} = info
            if (disabled) {
                return
            }

            let curTop = parseFloat(getComputedStyle(flake)['top'])
            let curLeft = parseFloat(getComputedStyle(flake)['left'])

            if (curLeft > window.innerWidth || curLeft < 0 || curTop > window.innerHeight) {
                // 删除
                document.body.removeChild(flake)
                flakeList.splice(i, 1)
            }
            curTop += verticalSpeed
            curLeft += horizontalSpeed
            flake.style.top = `${curTop}px`
            flake.style.left = `${curLeft}px`

        })
        requestAnimationFrame(update)
    }


    function generateFlake() {

        let flakeItem = {
            flake: null,
            verticalSpeed: null,
            horizontalSpeed: null,
            disabled: false
        }

        // 雪花字符 ❄❉❅❆✻✼❇❈❊✥✺

        // 距离 1-10 近-远
        flakeItem.distance = Math.floor(Math.random() * 10)

        // 偏移
        flakeItem.startLeft = Math.random() * window.innerWidth;
        // 偏移系数 [-10,10） 横向偏移距离=纵向偏移距离*offsetFactor
        flakeItem.verticalSpeed = (x => .5 * x + 4)(Math.random())
        flakeItem.horizontalSpeed = -flakeItem.verticalSpeed * (x => Math.tan(Math.pow(x, 2) * (Math.PI / 4)))(Math.random())
        flakeItem.size = 3 + 5 * flakeItem.distance;


        // 旋转
        flakeItem.spinReversed = Math.random() >= .5 ? true : false
        //[1,2)
        flakeItem.spinTime = 3


        flakeItem.flake = document.createElement('span');
        flakeItem.flake.innerHTML = '❅';
        flakeItem.flake.style.cssText = `
                        user-select:none;
                        position:fixed;
                        color:#fff;
                        left: ${flakeItem.startLeft}px;
                        font-size:${flakeItem.size}px;
                        top:-25px;
                        animation:${flakeItem.spinReversed ? 'spin-reversed' : 'spin'}  linear ${flakeItem.spinTime}s infinite;
                      `;

        // 随机变成袜子、礼物
        if (Math.random() < .2) {
            let t = Math.random()

            if (t < .333) {

                flakeItem.flake = document.createElement('img')
                flakeItem.flake.src = 'data:image/png;base64,'
                flakeItem.flake.style.cssText = `
                        user-select:none;
                        position:fixed;
                        left: ${flakeItem.startLeft}px;
                        width:50px;
                        top:-25px;
                        // animation:${flakeItem.spinReversed ? 'spin-reversed' : 'spin'}  linear ${flakeItem.spinTime}s infinite;
                        pointer-events:none;
                      `
                flakeItem.flake.ondragstart = function () {
                    return false
                }
            } else if (t < .666) {
                flakeItem.flake = document.createElement('img')
                flakeItem.flake.src = 'data:image/png;base64,'
                flakeItem.flake.style.cssText = `
                        user-select:none;
                        position:fixed;
                        left: ${flakeItem.startLeft}px;
                        width:50px;
                        top:-25px;
                        // animation:${flakeItem.spinReversed ? 'spin-reversed' : 'spin'}  linear ${flakeItem.spinTime}s infinite;
                        pointer-events:none;
                      `
                flakeItem.flake.ondragstart = function () {
                    return false
                }
            } else if (t < 1) {
                flakeItem.flake = document.createElement('img')
                flakeItem.flake.src = 'data:image/png;base64,'
                flakeItem.flake.style.cssText = `
                        user-select:none;
                        position:fixed;
                        left: ${flakeItem.startLeft}px;
                        width:50px;
                        top:-25px;
                        // animation:${flakeItem.spinReversed ? 'spin-reversed' : 'spin'}  linear ${flakeItem.spinTime}s infinite;
                        pointer-events:none;
                      `
                flakeItem.flake.ondragstart = function () {
                    return false
                }
            }

        }

        //拼接到页面中
        document.body.appendChild(flakeItem.flake);
        flakeList.push(flakeItem)

    }

    let flakeGenerator = function () {
        let t = 0
        return function () {
            t++
            if (t > 20) {
                generateFlake()
                t = 0
            }
            requestAnimationFrame(flakeGenerator)
        }
    }()
    requestAnimationFrame(update)
    requestAnimationFrame(flakeGenerator)
}

function bindEasterEgg(dom) {
    let node = document.createElement('div')
    node.innerHTML = `
        <style>
        .stage-1 {
            animation: stage-1 .5s forwards;
        }
    
        .stage-2 {
            animation: stage-2 .5s forwards;
        }
    
        .stage-3 {
            animation: stage-3 .5s forwards;
        }
    
        @keyframes stage-1 {
            0% {
                transform: scale(1);
            }
            50% {
                transform: scale(1.3);
            }
            100% {
                transform: scale(1.2);
            }
        }
    
        @keyframes stage-2 {
            0% {
                transform: scale(1.2);
            }
            50% {
                transform: scale(1.5);
            }
            100% {
                transform: scale(1.4);
            }
        }
    
        @keyframes stage-3 {
            0% {
                transform: scale(1.4);
                opacity: 1;
            }
            100% {
                transform: scale(3);
                opacity: 0;
                display: none;
                pointer-events: none;
            }
        }
    
        @keyframes spin {
            from {
                transform: rotateZ(0deg);
            }
            to {
                transform: rotateZ(360deg);
            }
        }
    
        @keyframes spin-reversed {
            from {
                transform: rotateZ(360deg);
            }
            to {
                transform: rotateZ(0deg);
            }
        }
    </style>
    `
    let christmasImg = document.createElement('img')
    christmasImg.src = "data:image/png;base64,"
    node.appendChild(christmasImg)

    let easterEggclickFunc = function () {
        let times = 0
        return function () {
            times++
            if (times > 3) {
                return
            } else if (times === 1) {
                christmasImg.classList.add('stage-1')
            } else if (times === 2) {
                christmasImg.classList.remove('stage-1')
                christmasImg.classList.add('stage-2')
            } else if (times === 3) {
                christmasImg.classList.remove('stage-2')
                christmasImg.classList.add('stage-3')
                setTimeout(() => {
                    snow();
                }, 1000)
            }
        }
    }()

    christmasImg.addEventListener('click', easterEggclickFunc)
    dom.parentNode.replaceChild(node, dom)

}

window.bindEasterEgg = bindEasterEgg